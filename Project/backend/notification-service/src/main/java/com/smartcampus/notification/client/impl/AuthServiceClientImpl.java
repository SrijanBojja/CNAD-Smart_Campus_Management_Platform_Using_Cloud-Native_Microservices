package com.smartcampus.notification.client.impl;
...
public class AuthServiceClientImpl implements AuthServiceClient {
    ...
    public UserValidationResponseDto validateUser(Long userId, String requiredRole, String bearerToken) {
        log.info("Validating Auth recipient user ID: {} with required role: {}", userId, requiredRole);
        ...
        try {
            var requestSpec = restClient.get()
                    .uri(uriBuilder -> {
                        var builder = uriBuilder.path("/api/v1/internal/users/{userId}/validation");
                        if (requiredRole != null && !requiredRole.trim().isEmpty()) {
                            builder.queryParam("requiredRole", requiredRole);
                        }
            ...
            return requestSpec.retrieve()
                    .onStatus(HttpStatusCode::is4xxClientError, (request, response) -> {
                        ...
                        if (statusCode == 404) {
                            ...
                            throw new BadRequestException("Recipient user with ID " + userId + " does not exist");
